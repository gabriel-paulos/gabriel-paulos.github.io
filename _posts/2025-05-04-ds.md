...
The frontier model writes code that generates a list of job specifications for the local (on-device) models to run in parallel. Each job specification, $$\textbf{t^i}$$, is a context-instruction pair, $$(q^i,c^i)$$, where $$q^i$$ is the task query for the job and $$c^i$$ is the partial context necessary to perform the task query. A question arises: how can the frontier LLM know what partial context to provide despite not having the full context within its memory? 
